                ...
                ?.run { name to parentFile.absolutePath.length }?.let { (srcRootName, relativePathHelperLength) ->
                    ...
                    """${
                        ...
                        codeFiles.joinToString("\n") {
                            ...
                            if (objectName.first()
                                    .isLowerCase()
                            ) """tasks.register("$taskTame"){ description = "from '$srcRootOfSimpleApp'"; group = "generated"; doLast { ${objectName}.main(arrayOf("${it.absolutePath}")) } }""" else ""
                        }
                    }
""".trim().takeIf { it.isNotEmpty() }?.let { generatedScriptText ->
    ...
    tasks.register("generatedDebugTask") {
